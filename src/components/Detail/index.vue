<template>
            ...
            <div class="choose">
              <div class="chooseArea">
                ...
                <dl v-for="spuSaleAttr in spuSaleAttrList " :key="spuSaleAttr.id">
                  ...
                  @click="changeAction(spuSaleAttrValue,spuSaleAttr.spuSaleAttrValueList)"> {{spuSaleAttrValue.saleAttrValueName}}</dd>
                  <!-- 这里拿到的是包夹在里面的那一层spuSaleAttrValueList数据 -->
                  <!-- 因为如果是点击进行修改，那么所修改的参数是在2层数据所包裹下的 所以就传2个参数 -->
                  </dl>
              </div>
              <div class="cartWrap">
                <div class="controls">
                  <input autocomplete="off" class="itxt" v-model="skuNum" @change="shopchange">
  ...
  <script>
    ...
    export default {
      ...
      methods: {
        changeAction(spuVal,spuValArr){
          spuValArr.forEach(item=>{
            item.isChecked ='0';
            spuVal.isChecked = '1';
          })
        },
        ...
        shopchange(){
          let value = this.shopnumber *1;
          if(isNaN(value) || value <1){
            this.shopnumber = 1;
          }else{
            this.shopnumber = parseInt(value);
          }
        },

        addShopcar(){
          
          // this.$store.dispatch('getShopcartData',{skuId:this.$route.params.skuid,skuNum:this.skuNum})
          try{
             this.$store.dispatch('getShopcartData',{
              skuId:this.$route.params.skuid,
              skuNum:this.skuNum
            });
            // 加入成功之后会进行路由的跳转，然后会把商品的数据传递到购物车中
            this.$router.push({name:'shopsuccess',query:{skuNum:this.skuNum}});
